...
const UploadProductsCSV = (props) => {
  // To push the downloadURL into the state array
  const { setCsvFiles } = useStateContext();
  ...
  useEffect(() => {
    const uploadFile = () => {
      const storageRef = ref(
        storage,
        `${appName}_${company}_${firstName}_userId:${userId}/store_csv/Products_CSV_${currentDate}_${file.name}`
      );
        ...
        {file === null ? (
          <>
            <LazyLoadImage
              ...
              src="https://media.publit.io/file/StockPics/OpenBox.svg"
              effect="blur"
              placeholderSrc="https://media.publit.io/file/StockPics/OpenBox.svg"
            />
            {/* <img
              src="https://media.publit.io/file/StockPics/OpenBox.svg"
              className="h-[70px] w-[70px]"
            ...
            <div className="hover:scale-105 transition-all duration-200">
              <TextIconBlueButton onClick={uploadFile} text={"Select a file"} />
            </div>
            <input
